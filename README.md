...
Este repositório é um material suplementar do texto "Uma abordagem geométrica da estabilidade local em dinâmicas discretas com aplicação em um modelo de cooperação", aqui está o código referente às simulações computacionais do modelo apresentado, utilizado para gerar a figura 7, na qual estas simulações são justapostas às equações mestras. Esse código está divido nas seguintes partes:
* Parâmetros
...
#IBM
Essa parte do código consiste nas simulações do modelo em si, visualizações dessas simulações feitas no R base (funções plot() e points()) e salva o output gerado para que ele possa ser utilizado em outros códigos e não precise ser gerado mais de uma vez. 
